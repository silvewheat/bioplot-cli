...
def load_vcf2array(vcffile, region: None, chrom2sites: None, samples, outsamples: None):
    ...
    # 只保留sitefile中的位点
    if chrom2sites:
        selection_requiredsites = [True if x in chrom2sites[chrom] else False for x in pos_array]
...
def main(vcffile, region, regionfile, sitefile, groupfile, groupcolor, outgroup, minMafGlobal, maxAltFreqs, minFreqDifs, outfile, dpi, figsize, outprefix, outsuffix):
    ...
    if outgroup:
        ...
        for group in outgroup.split(','):
            ...
            if len(samples) == 0:
                print(f'Warning: The outgroup "{group}" is not exist in the groupfile')
    ...
    for region in regions:
        ...
        # 读取vcf
        gt_array, samples_all, sites = load_vcf2array(vcffile, region, chrom2sites, samples_queried, samples_outgroup)
        ...
        if maxAltFreqs:
            ...
            for group, cutoff in maxAltFreqs:
                ...
                af_groupX = cal_ref_freq(gt_array[:, selection_samples, :]) # 这个返回的是ref的frequency
                selection_altfreq.append(af_groupX >= (1-cutoff))
            selection_altfreq = np.logical_and.reduce(selection_altfreq)
            selection_sites = np.logical_and(selection_sites, selection_altfreq)

        # 根据部分群体间ref频率差过滤
        if minFreqDifs:
            selection_freqdiff = []
            for group1, group2, cutoff in minFreqDifs:
                selection_group1 = select_samples(samples_all, group2samples[group1])
                selection_group2 = select_samples(samples_all, group2samples[group2])
            ...
            selection_sites = np.logical_and(selection_sites, selection_freqdiff)

        # 过滤得到最终的GT矩阵
        gt_array = np.sum(gt_array[selection_sites, :, :], axis=2) # 第三维度加和变二维度矩阵, 便于后面画图。./. -> -2, 0/1 -> 1, 1/1 -> 2 !按单倍型画的话，直接reshape展开就行了
